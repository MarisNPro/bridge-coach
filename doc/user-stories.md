...
**Acceptance criteria**
- `natural-v1.yaml` holds ~90 situations and ~558 priority-ordered rules; only the `conditions`
...
**Acceptance criteria**
- Engine: 219 pytest cases — parity, coverage (no null calls), HTTP-boundary for every endpoint
  incl. `/assess` and `/play`.
- Web: 59 Vitest tests — deal/generator logic (incl. trick-winner + DD projection), bridge rendering, system reference + weak-2 presets, interactive-play orchestration (hidden hands, bot opponents, declare/defend, auction constraints), the auction logic (contract derivation, constraint accumulation, framing/role/bot-call), and the bidding flow
...
- **Declare or Defend:** a toggle lets me either declare (play declarer + dummy) or defend as the
  opening leader, with the bot playing the whole declaring side.
...
- ◑ **Play polish & hidden-hand play** — _undo / last-trick / claim / animated compass / DD tracker,
  **hidden-hand play vs the non-cheating bot** (declare or defend), and **auction-aware (constraint)
  sampling** in the bot, **and a full bidding phase** (bid South vs bots → contract + constraints →
  bidding-aware play, with tap-to-explain calls) all shipped._ Residual: richer bidding depends on
  expanding the system data's auction coverage (a separate effort; gaps fall to pass + the adjust step).
